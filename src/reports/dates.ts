...
function chicagoParts(now: Date): { y: number; m: number; d: number } {
...
export type DateRange = { gte: string; lt: string };
...
export function thisYear(now: Date = new Date()): DateRange {
...
export function trailingDays(n: number, now: Date = new Date()): DateRange {
  const { y, m, d } = chicagoParts(now);
...
export function trailingMonths(n: number, now: Date = new Date()): DateRange {
  const { y, m, d } = chicagoParts(now);
