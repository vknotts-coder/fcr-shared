...
export function applyTrailerStatusEngine(before, edits, opts = {}) {
    ...
    const setStatus = (status, statusDate) => {
    ...
    if (opts.isNew) {
        ...
        const hasPickupAddr = ["pickup_address", "pickup_city", "pickup_state", "pickup_zip_code"].some((f) => !isBlank(eff(f)));
        let initial;
        if (driver === CUSTOMER_DROP_OFF)
            initial = "Received";
    ...
    if (advanced && (!manual || rank(advanced.status) >= rank(manual))) {
        if (rank(advanced.status) > rank(fromStatus))
            setStatus(advanced.status, advanced.date);
    }
    else if (manual) {
        setStatus(manual, todayStr);
        const dateField = STATUS_TO_DATE[manual];
        if (dateField && isBlank(eff(dateField)))
            derived[dateField] = todayStr;
    }
    const effStatus = derived.status ?? fromStatus;
