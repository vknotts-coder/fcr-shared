 ...
 */
export function diffChanges(before, after, opts = {}) {
    const numericCols = opts.numericCols ?? new Set();
    const dateCols = opts.dateCols ?? new Set();
    const datetimeCols = opts.datetimeCols ?? new Set();
    const exclude = opts.exclude ?? new Set();
    const keyAndPayload = (field, v) => {
        if (numericCols.has(field))
            return { key: toNum(v), payload: toNum(v) };
        if (dateCols.has(field))
            return { key: toDateOnly(v), payload: toDateOnly(v) };
        if (datetimeCols.has(field))
            return { key: toInstantMs(v), payload: toIso(v) };
...
const validators = [];
export function registerEventValidator(v) {
    validators.push(v);
}
...
function asUuid(name, v) {
    const t = (v ?? "").trim();
    if (!t)
        return null;
    if (!UUID_RE.test(t))
        throw new Error(`eventInsert: ${name} must be a uuid, got ${JSON.stringify(v)}`);
    return t;
}
/**
 ...
 */
export async function eventInsert(input, mutation) {
    ...
    for (const v of validators) {
        try {
            ...
            throw e instanceof EventValidationError
                ? e
                : new EventValidationError(e?.message ?? "event rejected by validator", { cause: e });
        }
    }
    ...
    const rows = input.changes && input.changes.length > 0
        ? input.changes
        : [{ field: null, before: undefined, after: undefined }];
 ...
 */
export async function commitWithEvent(input, mutation, exec) {
