...
function ident(seg) {
    if (!/^[a-z_][a-z0-9_]*$/i.test(seg))
        throw new Error(`unsafe identifier: ${seg}`);
    return `"${seg}"`;
}
...
function filterLeaf(obj, meta, fil, p) {
    ...
    if (fil.value == null || Array.isArray(fil.value))
        return null; // guarded by the validator
    if (meta.type === "date") {
...
function logicToSql(node, clauses) {
    if (node.op === "num")
        return clauses[node.n - 1] ?? "TRUE";
    if (node.op === "not")
        return `(NOT ${logicToSql(node.child, clauses)})`;
...
export function buildTabularQuery(obj, def, fieldsByKey, principal, rowCap = REPORT_ROW_CAP) {
    ...
    if (linkActive(obj))
        items.push(`${ident(obj.key)}.${ident(obj.link.path)} AS ${ident("__id")}`);
...
function aggSelectList(obj, def, fieldsByKey) {
 ...
 */
export function buildSummaryQuery(obj, def, fieldsByKey, principal, where = buildWhere(obj, def, fieldsByKey, principal)) {
...
export function buildSummaryTotalQuery(obj, def, fieldsByKey, principal, where = buildWhere(obj, def, fieldsByKey, principal)) {
    const aggs = aggSelectList(obj, def, fieldsByKey);
 ...
 */
export function validateReport(raw, principal, catalog) {
    const rawKey = typeof raw === "object" && raw !== null ? raw.object : undefined;
    if (typeof rawKey !== "string")
        return { ok: false, errors: ["missing object"] };
    const obj = getObjectDef(catalog, rawKey);
    if (!obj || !obj.capability(principal))
        return { ok: false, errors: ["object not available"] };
    ...
    try {
        if (def.summaries.length > 0) {
            ...
            const [grouped, total] = await Promise.all([
                db.query(gq.sql, gq.params),
                db.query(tq.sql, tq.params),
            ]);
...
function buildTabular(obj, def, fieldsByKey, rows, truncated) {
    ...
    for (const row of rows) {
        ...
        for (const c of columns)
            rec[c.key] = coerceCell(row[c.key], c.type);
        out.push(rec);
        if (linked) {
...
function summaryColumns(def, fieldsByKey) {
    return def.summaries.map((s) => {
        const meta = s.agg === "count" ? undefined : fieldsByKey.get(s.field);
        const type = s.agg !== "count" && meta?.type === "money" ? "money" : "number";
        return { key: summaryKey(s), label: summaryLabel(s.agg, meta?.label), type, numeric: true };
    });
}
export function sortSummaryRows(rows, columns, sort, groupKey) {
...
function readAgg(row, def) {
    const v = {};
    for (const s of def.summaries) {
        const key = summaryKey(s);
        const raw = row[key];
        if (s.agg === "count")
            v[key] = raw == null ? 0 : Number(raw);
        else if (s.agg === "sum")
            v[key] = raw == null ? 0 : round2(Number(raw));
        else if (s.agg === "avg")
            v[key] = raw == null ? null : round2(Number(raw));
        else
            v[key] = raw == null ? null : Number(raw); // min / max
    }
    return v;
}
...
function buildSummaryResult(obj, def, fieldsByKey, groupedRows, totalRow) {
    const groupMeta = def.groupBy ? fieldsByKey.get(def.groupBy.field) : undefined;
